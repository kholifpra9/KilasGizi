...
// 1. Pindahkan logika form yang menggunakan useSearchParams ke komponen internal
function LoginForm() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const redirectTo = searchParams.get('redirectTo') ?? '/generate-menu';
  ...
  return (
    <div className="w-full max-w-md rounded-3xl border border-kg-tan bg-card p-8 shadow-sm">
      ...
      <div className="mb-6 text-center">
        <Link
          href="/"
          className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-kg-green/10 text-2xl mb-3"
        >
          🥗
        </Link>
      ...
      <div className="mt-6 text-center border-t border-kg-tan/40 pt-4">
        ...
        >
          {mode === 'login' ? (
            <span>
              Belum punya akun? <strong className="text-kg-green underline">Daftar di sini</strong>
            </span>
          ) : (
            <span>
              Sudah punya akun? <strong className="text-kg-green underline">Masuk di sini</strong>
            </span>
          )}
        </button>
      </div>
...
// 2. Bungkus komponen dalam Suspense boundary pada default export
export default function LoginPage() {
  return (
    <div className="flex min-h-[calc(100vh-140px)] items-center justify-center bg-background px-4 py-12">
      <Suspense fallback={<div className="text-xs text-kg-ink/50">Memuat halaman...</div>}>
        <LoginForm />
      </Suspense>
    </div>
  );
